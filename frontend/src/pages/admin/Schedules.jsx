...
const Schedules = () => {
  ...
  const getRouteName = (routeId) => {
    ...
    return route ? `${route.route_name} (${route.start_location_name} - ${route.end_location_name})` : 'N/A';
  };

  const handleStatusChange = async (scheduleId, newStatus) => {
    try {
      await adminAPI.updateScheduleStatus(scheduleId, newStatus);
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {schedules.map((schedule) => (
                  <tr key={schedule.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {new Date(schedule.departure_time).toLocaleString('bn-BD')}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {new Date(schedule.arrival_time).toLocaleString('bn-BD')}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                ...
                >
                  ...
                  {routes.map((route) => (
                    <option key={route.id} value={route.id}>
                      {route.route_name} ({route.start_location_name} - {route.end_location_name})
                    </option>
                  ))}
                </select>
              </div>
              ...
              <div className="mb-4">
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    handleStatusChange(currentSchedule?.id, e.target.value);
                  }}
                  className="w-full border rounded px-3 py-2"
                >
